...
import { BroadcastCountdown } from "../features/player/components/BroadcastCountdown";
import { BroadcastMedia } from "../features/player/components/BroadcastMedia";
import { PlayerGuide } from "../features/player/components/PlayerGuide";
import { SubtitleSelector } from "../features/player/components/SubtitleSelector";
import { PLAYER_STATES } from "../features/player/constants/player.constants";
import { useBroadcastSession } from "../features/player/hooks/useBroadcastSession";
import { useKeyboardPlayerControls } from "../features/player/hooks/useKeyboardPlayerControls";
import { localMediaService } from "../features/player/services/local-media.service";
import { playerDb } from "../features/player/services/player-db.service";

const loadCinemaOpening = () => import("../features/player/components/CinemaOpening");
const CinemaOpening = lazy(() => loadCinemaOpening().then((module) => ({ default: module.CinemaOpening })));

function errorMessage(error) {
  return error?.message || "The feature could not be played.";
}
function sessionReturnPath(session) {
  if (session?.returnPath) return session.returnPath;
  if (session?.titleId && !session?.sourceKind) return "/categories/marvel";
  return "/watch-anything";
}
...
export default function PlayerPage() {
  ...
  useEffect(() => { playerDb.getLocalSource().then(setLocalSource).catch(() => setLocalSource(null)); }, []);
  ...
  if (player.loadingError || machine.status === PLAYER_STATES.ERROR) return <main className="wm-player-error"><h1>Unable to continue</h1><p>{errorMessage(player.loadingError || machine.error)}</p><Link className="secondary-button" to={sessionReturnPath(session)}>Return to library</Link></main>;
  if (!session) return <main className="wm-player-loading">Opening broadcast session…</main>;
  if (player.needsFile) return <main className="wm-player-error"><h1>Select the feature again</h1><p>Choose the same local file. Its name, size and modified date will be verified.</p>{player.recoveryError && <p role="alert">{player.recoveryError.message}</p>}<button className="primary-button" type="button" onClick={player.recover}>Select movie file</button><Link className="text-button" to={sessionReturnPath(session)}>Cancel</Link></main>;
  ...
  return <main className="wm-player" ref={containerRef}>
    ...
    {machine.status === PLAYER_STATES.COMPLETED && <div className="wm-ending"><h1>{session.mode === "test" ? "Testing finished." : "Feature presentation concluded."}</h1><p>{session.mode === "test" ? "Watched status and History were not changed." : session.sourceKind === "anything" ? "This local-only session was not added to History." : "This category title is marked watched and queued for History sync."}</p><Link className="primary-button" to={sessionReturnPath(session)}>Return to library</Link></div>}
