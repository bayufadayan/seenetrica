...
export function TestingStreamingButton({ settings, localSource, channels, toast }) {
  ...
  return <section className="wm-settings-section" id="testing"><header><div><p className="section-kicker">06 · Rehearsal</p><h2>Testing Streaming</h2></div></header><p>Run the same player with a 20-second pre-show, an early commercial break, and no changes to watched status.</p><button className="primary-button" type="button" disabled={busy} onClick={start}><FlaskConical /> {busy ? "Reading video…" : "Testing Streaming"}</button></section>;
}
