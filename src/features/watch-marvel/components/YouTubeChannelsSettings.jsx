...
export function YouTubeChannelsSettings({ channels, refresh, toast }) {
  ...
  async function refreshChannel(channel, quiet = false) {
    try {
  ...
  async function add(event) {
    ...
    try { const resolved = await youtubeTrailerService.resolveChannel(url); const channel = await watchMarvelDb.saveYouTubeChannel({ ...resolved, channelUrl: url }); await refreshChannel(channel, true); setUrl(""); await refresh(); toast("Trailer channel added."); } catch (error) { toast(error.message, "error"); } finally { setBusy(false); }
  }
