...
import { CategoryPrerequisiteSelector } from "./CategoryPrerequisiteSelector";
...
export function CategoryTitleForm({ existing, titles, categoryName, onSave, onClose }) {
  ...
  async function savePayload(payload) {
    setBusy(true);
    setError("");
    try { await onSave(payload); } catch (nextError) { setError(nextError.message); setBusy(false); }
  }
  function select(details, action) {
    setError("");
    if (action === "seasons") { setSeasonDetails(details); return; }
    setDraft(action === "movie" ? createMovieDraft(details) : createWholeSeriesDraft(details));
  }

  return (
    <Modal title={existing ? `Edit ${categoryName} title` : `Add ${categoryName} title`} onClose={onClose} busy={busy} className="wm-title-modal">
      {!existing && !draft && !seasonDetails && <TmdbSearchPanel getActions={actionsFor} onSelect={select} />}
      {!existing && seasonDetails && <CategorySeasonRangeForm details={seasonDetails} existingIdentityKeys={identityKeys} busy={busy} onCancel={() => setSeasonDetails(null)} onConfirm={savePayload} />}
      {draft && (
        <form className="wm-title-form" onSubmit={(event) => { event.preventDefault(); savePayload(draft); }}>
          ...
          <CategoryPrerequisiteSelector titles={titles} value={draft.prerequisiteIds || []} excludeId={existing?.id} disabled={busy} onChange={(prerequisiteIds) => setDraft({ ...draft, prerequisiteIds })} />
          {error && <p className="wm-field-error" role="alert">{error}</p>}
          <div className="form-actions"><button className="primary-button" disabled={busy}>{busy ? "Saving…" : "Save title"}</button>{!existing && <button className="text-button" type="button" disabled={busy} onClick={() => setDraft(null)}>Choose another</button>}</div>
        </form>
      )}
