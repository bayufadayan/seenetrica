...
describe("legacy atomic bulk creation", () => {
  beforeEach(() => { mock.state.data = []; });

  it("rejects an empty batch", async () => {
    await expect(playerDb.createTitles([])).rejects.toThrow("at least one title");
  });
  it("rejects duplicate identities within a batch", async () => {
    await expect(playerDb.createTitles([movie(1, "First"), movie(1, "Duplicate")])).rejects.toThrow("more than once");
  ...
  it("rejects a title already in the library", async () => {
    ...
    await expect(playerDb.createTitles([movie(1, "Existing")])).rejects.toThrow("already in the legacy library");
    expect(mock.state.data).toHaveLength(1);
  });
  it("saves every valid title in one batch", async () => {
    const saved = await playerDb.createTitles([movie(1), movie(2)]);
